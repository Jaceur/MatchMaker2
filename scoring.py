...
SEGMENT_TURNOVER_CAP = 30_000_000   # AEs sell to SMBs under £30m turnover
...
def _turnover_points(turnover, account_type):
    """Segment fit. Over the £30m cap scores nothing here (and is disqualified in
    score_lead). When turnover isn't filed — common for small companies — fall
    back to the account-size category rather than penalising them for it."""
...
def _substance_points(account_type, employee_count):
    ...
    if "small" in t or "medium" in t:
        ...
        size = 7
    elif "full" in t or "group" in t:    # likely larger / nearer the segment edge
        size = 5
    else:
        size = 0
    emp = 0
...
def _heuristic_score(f: LeadFeatures) -> int:
    ...
    # Hard disqualifiers — out of segment or not really trading.
    if "dormant" in t:
        return 0
    if f.turnover is not None and f.turnover > SEGMENT_TURNOVER_CAP:
        return 0
    score = (
